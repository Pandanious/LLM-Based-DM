...
def _format_npc_roster(npcs: Dict[str, NPC]) -> str:
    if not npcs:
        return "- (none listed)"
    lines = []
...
def _split_quest_chunks(text: str) -> List[str]:
...
def _resolve_location_name(world: World_State, raw_location: str) -> Optional[str]:
    """
...
def _find_npc_id_by_name(npcs: Dict[str, NPC], name: str) -> Optional[str]:
...
def generate_quests_for_world(
    ...
    """
    Use the LLM to generate a small set of structured quests for this world.
    Returns a dict quest_id -> Quest.
    """
    llm = get_llm()
    ...
    min_quests = max_quests  # guarantee at least this many, we can trim later if needed
